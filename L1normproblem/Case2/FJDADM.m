% use FJDADM proposed by He, Hou and Yuan in 2015 to solve the following problem
% min f(x):=||x||_1, s.t. Ax=b, lb¡Üx¡Üub,
% where A¡ÊR^{m¡Án}. 
% For this 

function out = FJDADM(xs,para)
...
%     end
    ...
    hat_x1 = P-Proj(-1./(beta*diag_AtA),1./(beta*diag_AtA),P);
    
    % check the stopping condition
    tused = toc;
    if max(norm(hat_x1-x0,inf),norm(pri_feas(hat_x1),inf))<epsilon || tused>100
        break;
    end
    
    % update multiplier
    hat_lam1 = lam0-beta*pri_feas(hat_x1);
    
    % generate the next iterate
    psi = beta*(norm(A*diag(x0-hat_x1),'fro')^2+norm(A*(x0-hat_x1))^2)+ ... 
        1/beta*norm(lam0-hat_lam1)^2;
    varphi = psi+2*(lam0-hat_lam1)'*A*(x0-hat_x1);
