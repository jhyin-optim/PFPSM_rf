function out = FJADMM(H,F,mu,opts)
...
for ii = 1:opts.maxitr
    ...
    %    Update Lam
    % ==================
    barLam1 = Lam1 - beta*Sum1;
    barLam2 = Lam2 - beta*Sum2;
    
    % Compute correction steplength
    phi = (norm(Sum1-Sum1p,'fro')^2+norm(Sum2-Sum2p,'fro')^2)+ ... 
        (norm(barY1-Y1,'fro')^2+norm(barY2-Y2,'fro')^2)+ ... 
        (norm(barD1X-D1X,'fro')^2+norm(barD2X-D2X,'fro')^2)+ ... 
        (norm(Sum1,'fro')^2+norm(Sum2,'fro')^2);
    varphi = 2*(Sum1(:)'*(Sum1p(:)-Sum1(:))+Sum2(:)'*(Sum2p(:)-Sum2(:)));
    alphak = gamma*(varphi/phi+1);
